...
import writhingmasscharactermod.cards.uncommon.Scarhide;
...
public class ScarhidePatch {
    @SpireInsertPatch(
            rloc=0
    )
    public static void updateScarhideBlock(AbstractCreature __instance)
    {
        if (__instance.equals(AbstractDungeon.player)) {
            ...
            for (CardGroup cardGroup : cardsInPlay) {
                for (int j = 0; j < cardGroup.getCardDeck().size(); j++) {
                    if (cardGroup.group.get(j).cardID.equals(Scarhide.ID)) {
                        int missingHP = ((AbstractPlayer)__instance).maxHealth - ((AbstractPlayer)__instance).currentHealth;
                        ((Scarhide) (cardGroup.group.get(j))).externalSetBlock(missingHP);
                    }
                }
            }
        }
